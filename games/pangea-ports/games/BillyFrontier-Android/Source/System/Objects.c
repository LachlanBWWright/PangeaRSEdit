...
float	max,n;
	...
	max = fabs(theNode->BBox.max.x);							// get radius to right
	n = fabs(theNode->BBox.min.x);								// get radius to left
	if (n > max)
		max = n;

	n = fabs(theNode->BBox.min.z);			// get radius to back
	if (n > max)
		max = n;
	n = fabs(theNode->BBox.max.z);			// get radius to front
	if (n > max)
		max = n;

	n = fabs(theNode->BBox.min.y);			// get radius to bottom
	if (n > max)
		max = n;
	n = fabs(theNode->BBox.max.y);			// get radius to top
	if (n > max)
		max = n;
...
int		i;
	...
	theNode->BoundingSphereRadius = gObjectGroupBSphereList[theNode->Group][theNode->Type] * theNode->Scale.x;


			/* IF HAD WORLD DATA, NUKE IT */
			
	for (i = 0; i < MAX_OBJECTS_IN_GROUP; i++)		// delete the arrays since the sizes may have changed with new object
	{
	...
	do
	{
		...
		if (theNode->CType == INVALID_NODE_FLAG)				// see if already deleted
			goto next;		
				...
				/* IF WE'RE IN PICKING MODE THEN WE NEED TO PASS A PICK ID */
...
//		glDepthMask(GL_TRUE);
    ...
    gGlobalMaterialFlags &= ~(BG3D_MATERIALFLAG_CLAMP_U|BG3D_MATERIALFLAG_CLAMP_V);	// wrapping ON
...
int		i;
	...
	DisposeObjectBaseGroup(theNode);					// dispose BG3D base group

	for (i = 0; i < MAX_OBJECTS_IN_GROUP; i++)			// delete world point arrays
	{
		if (theNode->WorldMeshes[i].points)
		{
			SafeDisposePtr(theNode->WorldMeshes[i].points);
...
/*********************** HIDE OBJECT **********************/
//
// Sets HIDE bit on node and all chains
//
...
ObjNode	*node = theNode;
	...
	while(node)
	{
		...
		if (node->ShadowNode)						// also hide shadows
...
/*********************** SHOW OBJECT **********************/
//
// Clears HIDE bit on node and all chains
//
...
ObjNode	*node = theNode;
	...
	while(node)
	{
		...
		if (node->ShadowNode)						// also show shadows
			node->ShadowNode->StatusBits &= ~STATUS_BIT_HIDDEN;
