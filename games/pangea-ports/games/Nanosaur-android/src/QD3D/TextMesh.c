...
void TextMesh_Init(void)
{
	...
	if (gFontTexture == 0)
	{
		SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "TextMesh_Init: Font texture 3000.tga not found. Debug text will be disabled.");
		return;
	}

	err = FSMakeFSSpec(gDataSpec.vRefNum, gDataSpec.parID, ":Images:Textures:3000.sfl", &spec);
	if (err != noErr)
	{
		SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "TextMesh_Init: Font metadata 3000.sfl not found. Debug text will be disabled.");
	...
	if (err != noErr)
	{
		SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "TextMesh_Init: Could not open 3000.sfl. Debug text will be disabled.");
		glDeleteTextures(1, &gFontTexture);
		gFontTexture = 0;
		return;
	}
...
ObjNode* TextMesh_Create(const TextMeshDef* def, const char* text)
{
	...
	// Attach color mesh
	AttachGeometryToDisplayGroupObject(textNode, 1, &mesh);
	...
	if (def->withShadow)
	{
		...
		AttachGeometryToDisplayGroupObject(shadowNode, 1, &shadowMesh);
