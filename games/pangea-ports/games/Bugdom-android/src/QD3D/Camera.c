...
void CalcCameraMatrixInfo(QD3DSetupOutputType *setupInfo)
{
	...
	Render_SetProjectionMatrix(&gCameraViewToFrustumMatrix);
	...
	Render_SetModelViewMatrix(&gCameraWorldToViewMatrix);

			/* UPDATE LIGHT POSITIONS (view-space directions) */

	Render_SetLights(&setupInfo->lightList, &gCameraWorldToViewMatrix);
