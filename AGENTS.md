...
Use Storybook to exercise actual user-facing application UI that users may encounter. Stories intended for layout, overflow, interaction, accessibility, or visual regression testing must mount the production components and compose them as they appear in the application, using realistic application state and representative bundled data or assets where available.
Do not add filler stories that merely render shadcn/Radix or other third-party UI primitives in isolation, restyle invented mock controls, or duplicate upstream component-library behavior. A primitive may have an isolated story only when the application materially customizes its behavior or appearance and that customization itself needs regression coverage.
Do not claim that a Storybook test covers an application screen, editor menu, or workflow when it substitutes fabricated markup for the production component. If a production component cannot be mounted in Storybook, document the concrete blocker and fix the required Storybook provider, loader, or fixture boundary instead of replacing the UI with a lookalike.
