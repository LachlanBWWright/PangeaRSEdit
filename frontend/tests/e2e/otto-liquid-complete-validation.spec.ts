...
test.describe("Otto Matic Liquid Bodies Validation", () => {
  test("should load Apocalypse level and verify 7 liquid bodies appear with correct logs", async ({
    ...
    const apocalypseFile = path.join(
      __dirname,
      "../../public/assets/ottoMatic/terrain/Apocalypse.ter.rsrc",
    );
    ...
    const foundPatchesLog = liquidLogs.find(
      (log) => log.includes("Found") && log.includes("liquid patches"),
    );
    expect(foundPatchesLog, "Should find liquid patches log").toBeTruthy();

    // Extract number of patches found
    const patchCountMatch = foundPatchesLog?.match(
      /Found (\d+) liquid patches/,
    );
    const patchCount = patchCountMatch
      ? parseInt(patchCountMatch[1] ?? "0", 10)
      : 0;
    ...
    expect(
      errors.length,
      `Should have no errors, but got: ${errors.join(", ")}`,
    ).toBe(0);
  ...
  test("should verify liquid Y-position is visible and correct", async ({
    ...
    const yPositionLogs = consoleLogs.filter(
      (log) => log.includes("Rendering patch") && log.includes("Y="),
    );
