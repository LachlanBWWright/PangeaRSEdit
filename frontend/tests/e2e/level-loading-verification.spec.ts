...
const gameTests = [
  ...
  },
  {
    ...
    levelPath: join(
      __dirname,
      "../../public/assets/bugdom2/terrain/Level1_Playroom.ter.rsrc",
    ),
  },
  {
    name: "Cro-Mag Rally",
    levelPath: join(
      __dirname,
      "../../public/assets/croMag/terrain/StoneAge_Jungle.ter.rsrc",
    ),
  },
];
...
test.describe("Level Loading Verification", () => {
  ...
  for (const game of gameTests) {
    test(`${game.name} - should load level and show editor UI`, async ({
      page,
    }) => {
      ...
      const loadButton = page
        .locator(`text=/Load.*Sample|Try.*Sample/i`)
        .first();
      ...
      if (!editorFound) {
        console.warn(
          `⚠ Editor UI not clearly visible for ${game.name}, but test continues`,
        );
      }
