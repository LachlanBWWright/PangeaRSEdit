...
test("Otto Matic level loads successfully with rsrcdump-ts", async ({
  page,
}) => {
  ...
  await page.screenshot({
    path: "test-results/debug-initial.png",
    fullPage: true,
  });
  ...
  // Take screenshot
  await page.screenshot({
    path: "test-results/debug-after-upload.png",
    fullPage: true,
  });
  ...
  if (pageText?.includes("Failed to parse")) {
    ...
    console.log(
      "Page excerpt:",
      pageText.substring(
        pageText.indexOf("Failed"),
        pageText.indexOf("Failed") + 200,
      ),
    );
  }
  ...
  const editorVisible = await page
    .locator('[data-testid="editor-view"]')
