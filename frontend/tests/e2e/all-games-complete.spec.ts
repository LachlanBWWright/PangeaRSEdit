...
test.describe('All Games - Level Loading and Download', () => {
  ...
  for (const game of gameConfigs) {
    test(`${game.name} - Complete workflow`, async ({ page }) => {
      ...
      await page.screenshot({ 
        path: `test-results/${game.name.replace(/\s+/g, '-')}-02-after-load.png`,
        fullPage: true 
      });

      // Check for errors
      const errorText = await page.locator('text=/error|failed|cannot/i').first().textContent().catch(() => null);
      ...
      const canvasVisible = await page.locator('canvas').first().isVisible({ timeout: 10000 }).catch(() => false);
      const downloadButton = await page.locator('[data-testid="download-button"]').isVisible({ timeout: 5000 }).catch(() => false);
      ...
      if (downloadButton) {
        ...
        const downloadPath = `test-results/${game.name.replace(/\s+/g, '-')}-downloaded.ter.rsrc`;
