/**
 * Nanosaur 2 Level Roundtrip Tests
 *
 * Tests byte-for-byte roundtrip accuracy for all Nanosaur 2 terrain files.
 */
...
describe("Nanosaur 2 Level Roundtrip", () => {
  const terrainDir = join(__dirname, "../../../../public/assets/nanosaur2/terrain");
  const levelFiles = [
    "battle1.ter.rsrc",
    "battle2.ter.rsrc",
    "flag1.ter.rsrc",
  ...
  for (const levelFile of levelFiles) {
    ...
    it(`should parse ${levelFile} to JSON with nanosaur2 specs`, () => {
      const filePath = join(terrainDir, levelFile);
      const originalData = readFileSync(filePath);
