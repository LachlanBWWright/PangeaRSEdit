...
import {
  parseSkeletonRsrcTS,
  parseSkeletonRsrcJson,
} from "./skeletonRsrc/parseSkeletonRsrcTS";
...
describe("Otto BG3D Skeleton Round-trip", () => {
  it("parses the Otto skeleton resource into raw JSON and logs it", () => {
    const ottoSkeletonPath = join(__dirname, "../../public/Otto.skeleton.rsrc");
    const ottoSkeletonData = readFileSync(ottoSkeletonPath);
    const parsed = parseSkeletonRsrcJson(
      new Uint8Array(ottoSkeletonData).buffer,
    );
    console.log(
      "Parsed Otto skeleton JSON:\n",
      JSON.stringify(parsed, null, 2),
    );
    expect(parsed).toBeDefined();
  });

  it("logs bones from the parsed Otto skeleton resource", () => {
    const ottoSkeletonPath = join(__dirname, "../../public/Otto.skeleton.rsrc");
    const ottoSkeletonData = readFileSync(ottoSkeletonPath);
    ...
    if (skeleton && skeleton.Bone) {
      Object.entries(skeleton.Bone).forEach(([id, entry]) => {
        const obj: any = entry.obj as any;
        console.log(
          `Bone id=${id} name=${entry.name} order=${entry.order} parent=${obj?.parentBone} coords=[${obj?.coordX}, ${obj?.coordY}, ${obj?.coordZ}]`,
        );
      });
    } else {
      console.log("No Bone resources found in skeleton");
    }

    // RelP logging moved to a dedicated test

    expect(skeleton).toBeDefined();
  });

  it("logs ALL RelP (relative points) from the parsed Otto skeleton resource", () => {
    const ottoSkeletonPath = join(__dirname, "../../public/Otto.skeleton.rsrc");
    const ottoSkeletonData = readFileSync(ottoSkeletonPath);
    const skeleton = parseSkeletonRsrcTS(
      new Uint8Array(ottoSkeletonData).buffer,
    ...
    console.log("\n=== ALL Otto Skeleton RelP (relative points) ===");
    if (skeleton && skeleton.RelP) {
      Object.entries(skeleton.RelP).forEach(([id, entry]) => {
        const pts: any[] = entry.obj as any[];
        console.log(
          `RelP id=${id} name=${entry.name} order=${entry.order} pointCount=${pts.length}`,
        );
        for (let i = 0; i < pts.length; i++) {
          const p = pts[i];
          console.log(
            `  ${i}: [${p?.relOffsetX}, ${p?.relOffsetY}, ${p?.relOffsetZ}]`,
          );
        }
      });
    } else {
      console.log("No RelP resources found in skeleton");
    }
  ...
  it("should preserve Otto animation timing and structure in round-trip conversion", async () => {
    ...
    console.log(
      "Parsing Otto skeleton resource with TypeScript implementation...",
    );
    const skeleton = parseSkeletonRsrcTS(
      new Uint8Array(ottoSkeletonData).buffer,
    );
    ...
    console.log(
      `Joints accessible from scene: ${accessibleJoints}/${joints.length}`,
    );
    ...
    animations.forEach((animation) => {
      ...
      console.log(
        `Checking animation "${animation.getName()}" with ${
          channels.length
        } channels`,
      );
      ...
      channels.forEach((channel, channelIndex) => {
        ...
        console.log(
          `  Channel ${channelIndex}: ${targetNode?.getName()}.${targetPath}`,
        );
      ...
      console.log(
        `Animation "${animation.getName()}" max duration: ${maxDuration}`,
      );
    ...
    Object.entries(animationDurations).forEach(([name, duration]) => {
      if (duration > 0) {
        ...
        console.log(
          `Animation "${name}": ${duration.toFixed(
            3,
          )} seconds (single-frame pose)`,
        );
      }
    });
    ...
    console.log(
      `Valid animations with timing: ${validAnimationCount}/${animations.length}`,
    );

    // Convert back to skeleton resource format
    console.log("Converting back to skeleton resource format...");
    const reconstructedSkeleton = bg3dSkeletonToSkeletonResource(
      bg3dParsed.skeleton!,
    );
    ...
    const originalAnimCount = skeleton.AnHd
      ? Object.keys(skeleton.AnHd).length
      : 0;
    const reconstructedAnimCount = reconstructedSkeleton.AnHd
      ? Object.keys(reconstructedSkeleton.AnHd).length
      : 0;

    expect(reconstructedAnimCount).toBe(originalAnimCount);

    console.log(
      `Original animations: ${originalAnimCount}, Reconstructed: ${reconstructedAnimCount}`,
    );
