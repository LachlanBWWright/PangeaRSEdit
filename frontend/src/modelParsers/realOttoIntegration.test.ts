...
describe("Real Otto Integration", () => {
  it("should test real Otto BG3D + skeleton parsing", async () => {
    ...
    console.log(
      `3. KeyF resources available: ${Object.keys(skeleton.KeyF || {}).length}`,
    );
    ...
    try {
      ...
      console.log(`   Skeleton: ${parsed.skeleton ? "Present" : "Missing"}`);

      if (parsed.skeleton) {
        console.log(`   Bones: ${parsed.skeleton.bones.length}`);
        console.log(`   Animations: ${parsed.skeleton.animations.length}`);

        // Check animation durations
        parsed.skeleton.animations.forEach((anim, index) => {
          const totalKeyframes = Object.values(anim.keyframes).reduce(
          ...
          if (totalKeyframes > 0) {
            ...
            console.log(
              `     Animation ${index}: ${
                anim.name
              } - ${totalKeyframes} keyframes, duration: ${duration.toFixed(
                2,
              )}s`,
            );
          } else {
