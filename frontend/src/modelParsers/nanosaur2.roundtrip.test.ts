...
describe('Nanosaur 2 Round-trip Parsing Tests', () => {
  const basePath = join(__dirname, '..', '..', 'public', 'PangeaRSEdit', 'games', 'nanosaur2', 'skeletons');
  ...
  test('Nanosaur 2 bone hierarchy comparison', async () => {
    ...
    const nanoResource = parseSkeletonRsrcTS(nanoBuffer);
    const raptorResource = parseSkeletonRsrcTS(raptorBuffer);
    ...
    const nanoData = parseBG3D(nanoBg3dBuffer, nanoResource);
    const raptorData = parseBG3D(raptorBg3dBuffer, raptorResource);
    ...
    const nanoBoneNames = nanoData.skeleton!.bones.map(bone => bone.name.toLowerCase());
    const raptorBoneNames = raptorData.skeleton!.bones.map(bone => bone.name.toLowerCase());
    ...
    for (const expectedBone of expectedBones) {
      const nanoHasBone = nanoBoneNames.some(name => name.includes(expectedBone));
      const raptorHasBone = raptorBoneNames.some(name => name.includes(expectedBone));
    ...
    console.log(`Nano bones: ${nanoData.skeleton!.bones.map(b => b.name).join(', ')}`);
    console.log(`Raptor bones: ${raptorData.skeleton!.bones.map(b => b.name).join(', ')}`);
  });
  ...
  test('Nanosaur 2 animation system validation', async () => {
    ...
    if (bg3dData.skeleton!.animations && bg3dData.skeleton!.animations.length > 0) {
      const doc = new Document();
      const buffer = doc.createBuffer();
      ...
      console.log(`Nano animations: ${animations.map(a => a.getName()).join(', ')}`);
    }
  });
  ...
  test('BG3D JPEG texture extraction for Nanosaur 2', async () => {
    ...
    if (jpegTextureCount > 0) {
      console.log(`✅ Worm model contains ${jpegTextureCount} JPEG textures (${totalTextureSize} bytes total)`);
    } else {
  ...
  test('Nanosaur 2 bone transformation matrices', async () => {
    ...
    console.log(`Brach skeleton: validated ${joints.length} joint transformations`);
  });
  ...
  test('Nanosaur 2 round-trip coordinate preservation', async () => {
    ...
    for (let i = 0; i < Math.min(joints.length, bg3dData.skeleton!.bones.length); i++) {
      const joint = joints[i];
      ...
      const tolerance = 0.1;
      expect(Math.abs(translation[0] - originalBone.coordX)).toBeLessThan(tolerance);
      expect(Math.abs(translation[1] - originalBone.coordY)).toBeLessThan(tolerance);
      expect(Math.abs(translation[2] - originalBone.coordZ)).toBeLessThan(tolerance);
    }
    
    console.log(`Ramphor: coordinate preservation validated for ${joints.length} bones`);
  });
});