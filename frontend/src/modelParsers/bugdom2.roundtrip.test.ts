...
describe("Bugdom 2 Round-trip Parsing Tests", () => {
  ...
  const testModels = ["Ant", "Checkpoint", "Chipmunk", "MouseTrap", "BuddyBug"];
  ...
  test("Bugdom 2 skeleton hierarchy validation", async () => {
    ...
    const bonesWithParents = bg3dData.skeleton!.bones.filter(
      (bone) => bone.parentBone !== -1,
    );
  ...
  test("Bugdom 2 animation data preservation", async () => {
    ...
    if (bg3dData.skeleton!.animations.length > 0) {
      ...
      console.log(
        `Checkpoint animations: ${animations
          .map((a) => a.getName())
          .join(", ")}`,
      );
    }
  ...
  test("Bugdom 2 bone coordinate system consistency", async () => {
    ...
    console.log(
      `Chipmunk bone coordinates validated for ${
        bg3dData.skeleton!.bones.length
      } bones`,
    );
  });
});
