...
describe("BG3D + Skeleton Roundtrip Tests", () => {
  // Test data paths
  const ottoBg3dPath =
    "/home/runner/work/PangeaRSEdit/PangeaRSEdit/frontend/public/Otto.bg3d";
  const ottoSkeletonPath =
    "/home/runner/work/PangeaRSEdit/PangeaRSEdit/frontend/public/Otto.skeleton.rsrc";
  ...
  it("should perform complete BG3D+skeleton binary roundtrip with data integrity", async () => {
    ...
    console.log(
      "Original skeleton resource structure:",
      Object.keys(originalSkeletonResource),
    );
    ...
    expect(roundtripResult.skeleton!.animations.length).toBe(
    ...
    const originalKeys = Object.keys(originalSkeletonResource).filter((key) =>
      skeletonResourceTypes.includes(key),
    );
    const roundtripKeys = Object.keys(reparsedSkeletonResource).filter((key) =>
      skeletonResourceTypes.includes(key),
    );
      ...
      for (const channel of channels) {
        ...
        if (sampler) {
          ...
          if (times && times.length > 1) {
            ...
            console.log(
              `Animation ${animation.getName()}: duration = ${duration.toFixed(
                3,
              )}s`,
            );
          }
