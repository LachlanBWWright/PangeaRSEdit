  ...
  it("should create valid glTF with properly structured skeleton animations", async () => {
    ...
    animations.forEach((animation) => {
      ...
      if (animations.indexOf(animation) < 3) {
        console.log(`  Animation ${animations.indexOf(animation)}: "${animation.getName()}"`);
        ...
        channels.forEach((channel, channelIndex) => {
          ...
          if (channelIndex < 5) { // Log first 5 channels
            console.log(`      Channel ${channelIndex}: ${targetNode?.getName()}.${targetPath} (sampler: ${sampler ? "✓" : "✗"})`);
          }
        });
      }
    });
    ...
    joints.forEach((joint, index) => {
      ...
      if (index < 5) { // Log first 5 joints
        console.log(`  Joint "${joint.getName()}": ${isAccessible ? "✓ accessible" : "✗ not accessible"}`);
      }
    });
    
    console.log(`Joints accessible from scene: ${jointsAccessibleFromScene}/${joints.length}`);
    ...
    console.log("✓ All joints are accessible from scene (PropertyBinding compatible)");
