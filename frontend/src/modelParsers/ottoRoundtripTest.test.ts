...
describe("Otto Complete Roundtrip Tests", () => {
  ...
  testModels.forEach(({ name, bg3dPath, skeletonPath }) => {
    it(`should perform byte-for-byte roundtrip of ${name}.bg3d + ${name}.skeleton.rsrc`, async () => {
      ...
      console.log(
        `${name} - Original BG3D size: ${originalBg3dData.length} bytes`,
      );
      console.log(
        `${name} - Original skeleton size: ${originalSkeletonData.length} bytes`,
      );
      ...
      console.log(
        `${name} - glTF animations created: ${gltfAnimations.length}`,
      );
      ...
      const animationInfo = gltfAnimations.map((anim) => {
        ...
        let maxTime = 0;
        console.log(
          `Animation "${anim.getName()}" has ${channels.length} channels`,
        );
          ...
          if (sampler) {
            ...
            if (input) {
              ...
              if (times && times.length > 0) {
                ...
                console.log(
                  `  Channel ${channelIndex}: ${times.length} time points, max time = ${channelMaxTime}`,
                );
      ...
      const multiFrameAnimations = animationInfo.filter(
        (anim) => anim.duration > 0 || anim.channelCount > 0,
      );
      ...
      console.log(
        `${name} - Roundtrip BG3D size: ${roundtripBg3dBinary.byteLength} bytes (original: ${originalBg3dData.length})`,
      );
      console.log(
        `${name} - Roundtrip skeleton size: ${roundtripSkeletonBinary.byteLength} bytes (original: ${originalSkeletonData.length})`,
      );
      ...
      console.log(
        `${name} - BG3D accuracy: ${(bg3dAccuracy * 100).toFixed(
          6,
      ...
      expect(bg3dAccuracy).toBeGreaterThan(0.9); // Should be at least 90% accurate

      console.log(
        `=== ${name} Complete Roundtrip Test Completed Successfully ===`,
      );
    });
  });
  ...
  it("should test all models using glTF validator", async () => {
    ...
    for (const { name, bg3dPath, skeletonPath } of testModels) {
      ...
      // Run actual glTF validator to ensure 0 errors
      const validationReport = await validateBytes(new Uint8Array(glbBuffer));

      // Check for validation errors
      const errorIssues =
        validationReport.issues.messages.filter(
      ...
      console.log(
        `${name} - glTF validation: ${validationReport.issues.numErrors} errors, ${validationReport.issues.numWarnings} warnings`,
      );

      // Log any issues for debugging
      if (errorIssues.length > 0) {
        console.error(`${name} - Validation errors:`);
        errorIssues.forEach((error: any) =>
          console.error(`  ${error.message}`),
        );
      }

      if (warningIssues.length > 0) {
        console.warn(`${name} - Validation warnings:`);
        warningIssues.forEach((warning: any) =>
          console.warn(`  ${warning.message}`),
        );
      }

      // Require 0 validation errors for full accuracy
      expect(validationReport.issues.numErrors).toBe(0);
      expect(glbBuffer.byteLength).toBeGreaterThan(0);
      console.log(
        `${name} - GLB export size: ${glbBuffer.byteLength} bytes - ✓`,
      );
      ...
      console.log(
        `${name} - glTF structure: ${scenes.length} scenes, ${animations.length} animations, ${skins.length} skins - ✓`,
      );
    }
  ...
  it("should preserve animation timing data accurately", async () => {
    ...
    const originalTimingInfo: {
      [name: string]: { maxTick: number; keyframeCount: number };
    } = {};
          ...
          if (input) {
            ...
            if (times && times.length > 0) {
              maxDuration = Math.max(
                maxDuration,
                times[times.length - 1] as number,
              );
            }
          }
        }
    ...
    Object.entries(gltfTimingInfo).forEach(([name, info]) => {
      if (
        originalTimingInfo[name] &&
        originalTimingInfo[name].keyframeCount > 0
      ) {
        ...
        if (originalTimingInfo[name].keyframeCount > 1) {
          console.log(
            `Checking animation "${name}": ${originalTimingInfo[name].keyframeCount} keyframes, ${info.duration} duration`,
          );
          if (info.duration > 0) {
            validAnimationCount++;
          } else {
            console.warn(
              `Animation "${name}" has ${originalTimingInfo[name].keyframeCount} keyframes but 0 duration - possible timing issue`,
            );
          }
        }
      }
  ...
  it("should preserve bone hierarchy and transformations", async () => {
    ...
    for (let i = 0; i < originalBones.length; i++) {
      ...
      expect(Math.abs(roundtripBone.coordX - originalBone.coordX)).toBeLessThan(
        0.001,
      );
      expect(Math.abs(roundtripBone.coordY - originalBone.coordY)).toBeLessThan(
        0.001,
      );
      expect(Math.abs(roundtripBone.coordZ - originalBone.coordZ)).toBeLessThan(
        0.001,
      );

      console.log(
        `Bone "${originalBone.name}": ✓ hierarchy and coordinates preserved`,
      );
    }
