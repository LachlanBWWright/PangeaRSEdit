...
export function extractTileInfo(terrainData: TerrainData): TileInfo[] {
  ...
  for (let attrIndex = 0; attrIndex < atrb.length; attrIndex++) {
    ...
    const baseTileNumber =
      attr.bits !== undefined ? extractTileNumber(attr.bits) : tileIndex;
...
export function getTileUsageStats(
  ...
  const sortedByUsage = [...usedTiles].sort(
...
export function findTilePositions(
  ...
  for (let z = 0; z < header.mapHeight; z++) {
    for (let x = 0; x < header.mapWidth; x++) {
      const cellIndex = z * header.mapWidth + x;
      if (layr[cellIndex] !== attributeIndex) continue;
      positions.push({ x, z });
    }
  }
 ...
 */
export function getUnusedTileIndices(terrainData: TerrainData): number[] {
  const tiles = extractTileInfo(terrainData);
  return tiles.filter((t) => t.usageCount === 0).map((t) => t.attributeIndex);
}
