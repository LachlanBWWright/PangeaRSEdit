...
import type { Resource, ResourceFork, ResourceConverter, StructTemplate } from './types';
...
export class StructConverter implements ResourceConverter {
  ...
  static fromTemplateStringWithTypename(templateArg: string): Result<[StructConverter | null, Uint8Array | null], Error> {
    ...
    const templateResult = StructTemplateParser.fromTemplateString(formatAndFields);
  ...
  unpack(resource: Resource, _fork?: ResourceFork): Result<any, Error> {
  ...
  pack(_obj: any): Result<Uint8Array, Error> {
    return err(new Error('JSON->Binary packing not implemented in StructConverter'));
  }
}
