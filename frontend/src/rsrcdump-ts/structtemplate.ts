...
export class StructTemplateParser {
  ...
  private expandFieldNames(fields: string[]): (string | null)[] {
    ...
    while (
      ...
      fieldNameIndex < this.fieldNames.length
    ) {
      ...
      if (fieldName && fieldName.includes("[") && fieldName.includes("]")) {
        const expandedNames = this.expandArrayFieldNames(
          fieldName,
          fieldIndex,
          fields,
        );
  ...
  private static splitStructFormatFieldsStatic(
    fmt: string,
  ): Result<string[], Error> {
