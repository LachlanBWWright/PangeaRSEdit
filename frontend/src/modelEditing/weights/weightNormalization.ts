import type { SkinWeightsData, VertexWeightInfo, VertexWeight } from "./weightTypes";
 ...
 */
export function normalizeVertexWeights(vertex: VertexWeightInfo): VertexWeightInfo {
  // Prune tiny weights
  ...
  const sorted = [...pruned].sort((a, b) => b.weight - a.weight).slice(0, MAX_INFLUENCES);

  // Normalize
  const total = sorted.reduce((sum, w) => sum + w.weight, 0);
  if (total <= 0) return { ...vertex, influences: [] };
  const normalized = sorted.map((w): VertexWeight => ({ ...w, weight: w.weight / total }));
...
export function findUnweightedVertices(data: SkinWeightsData): number[] {
  return data.vertices
    .filter((v) => v.influences.reduce((sum, w) => sum + w.weight, 0) <= MIN_WEIGHT)
    .map((v) => v.vertexIndex);
}
 ...
 */
export function findNormalizationErrors(data: SkinWeightsData, tolerance = 0.01): number[] {
  return data.vertices
    .filter((v) => {
 ...
 */
export function repairNormalizationErrors(data: SkinWeightsData): SkinWeightsData {
