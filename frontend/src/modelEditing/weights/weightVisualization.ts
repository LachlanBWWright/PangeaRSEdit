    ...
    switch (mode) {
      case "heatmap": {
        ...
        const boneWeight = targetBone
          ? (vertex.influences.find((w) => w.boneName === targetBone)?.weight ?? 0)
      ...
      case "dominant": {
        ...
        const dominant = vertex.influences.reduce((best, w) => (w.weight > best.weight ? w : best));
...
function heatmapColor(t: number): [number, number, number] {
  ...
  if (clamped < 0.5) {
    return [0, Math.round(clamped * 2 * 255), Math.round((1 - clamped * 2) * 255)];
  }
  return [Math.round((clamped - 0.5) * 2 * 255), Math.round((1 - (clamped - 0.5) * 2) * 255), 0];
}

// Visually distinct colors for up to 32 bones
const BONE_COLORS: [number, number, number][] = [
  [255, 100, 100], [100, 255, 100], [100, 100, 255], [255, 255, 100],
  [255, 100, 255], [100, 255, 255], [255, 180, 50], [180, 50, 255],
  [50, 255, 180], [255, 50, 180], [180, 255, 50], [50, 180, 255],
  [200, 200, 80], [80, 200, 200], [200, 80, 200], [150, 220, 100],
  [220, 150, 100], [100, 150, 220], [220, 100, 150], [100, 220, 150],
  [255, 160, 122], [135, 206, 235], [144, 238, 144], [255, 215, 0],
  [210, 180, 140], [176, 196, 222], [240, 128, 128], [152, 251, 152],
  [173, 216, 230], [255, 182, 193], [250, 128, 114], [189, 183, 107],
];
