 ...
 */
export function AnimationUpdater({ animationMixer, logBonePositions = false }: AnimationUpdaterProps) {
  ...
  useEffect(() => {
    if (mixerRef.current && logBonePositions) {
      ...
      if (bones.length > 0) {
        ...
        console.log(`Tracking ${bones.length} bones:`, bones.map(b => b.name));
      }
    } else {
      setBonesTracked([]);
    }
  }, [mixerRef.current, logBonePositions]);
    ...
    if (mixerRef.current) {
      ...
      if (logBonePositions && bonesTracked.length > 0) {
        ...
        if (frameCountRef.current % logIntervalRef.current === 0) {
          ...
          console.log(`=== Frame ${frameCountRef.current} (t=${currentTime.toFixed(2)}s) ===`);
