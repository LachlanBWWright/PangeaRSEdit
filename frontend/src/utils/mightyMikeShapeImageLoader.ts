...
async function loadShapesFile(
...
): Promise<Result<ShapesFile>> {
  // Check cache first
  const cachedShapes = shapesFileCache.get(shapesFilename);
  if (cachedShapes) {
    return ok(cachedShapes);
  }
  ...
  try {
    ...
    if (!response.ok) {
      return err(
        new Error(`HTTP ${String(response.status)}: ${response.statusText}`),
      );
    }
...
function getFrameCacheKey(
  ...
  return `${shapesFile}:${String(shapeIndex)}:${String(frameIndex)}`;
}
...
async function loadShapeFrame(
  ...
  // Check cache first
  const cachedCanvas = frameCanvasCache.get(cacheKey);
  if (cachedCanvas) {
    return ok(cachedCanvas);
  }
  ...
  try {
    ...
    if (shapeIndex < 0 || shapeIndex >= shapesFile.shapes.length) {
      return err(
        new Error(
          `Shape index ${String(shapeIndex)} out of bounds (file has ${String(
            shapesFile.shapes.length,
          )} shapes)`,
        ),
      );
    }
    ...
    if (!shape) {
      return err(
        new Error(`Shape ${String(shapeIndex)} not found in shapes file`),
      );
    }
    ...
    if (frameIndex < 0 || frameIndex >= shape.frames.length) {
      return err(
        new Error(
          `Frame index ${String(frameIndex)} out of bounds (shape has ${String(
            shape.frames.length,
          )} frames)`,
        ),
      );
    ...
    if (!frame) {
      return err(
        new Error(
          `Frame ${String(frameIndex)} not found in shape ${String(
            shapeIndex,
          )}`,
        ),
      );
    ...
    const originalCanvasResult = shapeFrameToCanvas(
      frame,
      shapesFile.colorTable,
    );
...
export async function loadItemImage(
  ...
  try {
    ...
    return err(
      new Error(
        `Failed to load item ${String(itemType)} image: ${
          error instanceof Error ? error.message : String(error)
        }`,
      ),
    );
...
export async function preloadItemImages(
  ...
  if (failures.length > 0) {
    return err(
      new Error(
        `Failed to preload ${String(failures.length)} item image(s): ${failures
