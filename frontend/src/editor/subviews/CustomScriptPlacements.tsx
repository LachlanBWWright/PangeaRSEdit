...
export const CustomScriptPlacements = memo(function CustomScriptPlacements({
  ...
  const [selectedPlacementId, setSelectedPlacementId] = useAtom(SelectedCustomPlacementAtom);
  ...
  const handleDragEnd = useCallback(
    (placementId: string, x: number, z: number) => {
      setWorkspaceStore((store) => {
        const updatedWorkspace = moveCustomPlacement(
          workspace,
          placementId,
          {
            x,
            y: currentLevel?.customPlacements.find((placement) => placement.id === placementId)?.position.y ?? 0,
            z,
          },
        );
