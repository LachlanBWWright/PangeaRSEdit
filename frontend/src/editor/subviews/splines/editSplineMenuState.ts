import type { SplineData } from "@/python/structSpecs/LevelTypes";

export function createReindexedRecord<T extends { obj: unknown }>(
  record: Record<number, T>,
  removedIndex: number,
  keyBase = 0,
): Record<number, T> {
  const removedKey = keyBase + removedIndex;
  const entries = Object.entries(record)
    .map(([key, value]) => [Number(key), value] as const)
    .filter(([key]) => key !== removedKey)
    .sort(([left], [right]) => left - right);

  return Object.fromEntries(
    entries.map(([key, value]) => [key > removedKey ? key - 1 : key, value]),
  );
}

export function removeSplineAtIndex(
  splineData: SplineData,
  splineIdx: number,
): void {
  const splines = splineData.Spln?.[1000]?.obj;
  if (!splines || splineIdx < 0 || splineIdx >= splines.length) {
    return;
  }

  splines.splice(splineIdx, 1);

  if (splineData.SpNb) {
    splineData.SpNb = createReindexedRecord(splineData.SpNb, splineIdx, 1000);
  }
  if (splineData.SpPt) {
    splineData.SpPt = createReindexedRecord(splineData.SpPt, splineIdx, 1000);
  }
  if (splineData.SpIt) {
    splineData.SpIt = createReindexedRecord(splineData.SpIt, splineIdx, 1000);
  }

  const splineObjs = splineData.Spln?.[1000]?.obj;
  if (!splineObjs) return;
  splineObjs.forEach((spline, index) => {
    spline.numNubs =
      splineData.SpNb?.[1000 + index]?.obj.length ?? spline.numNubs;
    spline.numPoints =
      splineData.SpPt?.[1000 + index]?.obj.length ?? spline.numPoints;
    spline.numItems =
      splineData.SpIt?.[1000 + index]?.obj.length ?? spline.numItems;
  });
}
